  ...
  return <section className="py-16 bg-white">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {categories.map((category, index) => <Link key={category.title} to={category.linkTo}>
              <Card className={`${category.bgColor} border-0 shadow-lg hover:shadow-xl transition-all duration-300 hover-lift cursor-pointer group animate-fade-in relative overflow-hidden`} style={{
            animationDelay: `${index * 0.1}s`
          }}>
                {category.isHotSale && <Link to="/products?sale=true" className="absolute top-3 right-3 z-10" onClick={e => e.stopPropagation()}>
                ...
                <CardContent className="p-6 text-center">
                  <div className="mb-4">
                    <img src={category.image} alt={category.title} className="w-full h-32 object-cover rounded-lg mb-4 group-hover:scale-105 transition-transform duration-200" loading="lazy" decoding="async" />
                  </div>
